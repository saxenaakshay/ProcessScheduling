...
int	epochstate;		/* maintains current state of epoch	*/
int	schedclass;		/* maintains current scheduler class	*/
...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pprio = 0;
	currpid = NULLPROC;
	epochstate = 0;			/* initialize epoch with 0, completed/not running */
	schedclass = 3;
